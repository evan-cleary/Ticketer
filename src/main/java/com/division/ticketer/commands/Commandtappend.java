 ...
 * @author Evan
 */
public class Commandtappend extends TicketerCommand {
    ...
    public void run(Ticketer TI, Player sender, String commandLabel, Command command, String[] args) {
        ...
        if (args.length > 0) {
            if (tick != null) {
                tick.appendMessage(stringBuilder(args));
                sender.sendMessage(rankformat + ": " + chatformat + "You added '" + stringBuilder(args) + "' to your message.");
            } else {
                sender.sendMessage(rankformat + ": " + chatformat + "You do not have a pending ticket. Use /tcreate to start a ticket.");
            }
        } else {
            sender.sendMessage(rankformat + ": " + chatformat + "You need to specifiy what you want to add.");
        }
    }
}
